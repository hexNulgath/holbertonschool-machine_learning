#!/usr/bin/env python3
"""
Plotting two curves on the same graph.
This script demonstrates how to plot two curves
on the same graph using Matplotlib.
"""
import numpy as np
import matplotlib.pyplot as plt


def two():
    """Plot two curves on the same graph."""
    x = np.arange(0, 21000, 1000)
    r = np.log(0.5)
    t1 = 5730
    t2 = 1600
    y1 = np.exp((r / t1) * x)
    y2 = np.exp((r / t2) * x)

    plt.figure(figsize=(8, 6))
    plt.xlabel("Time (years)", fontsize=12)
    plt.ylabel("Fraction Remaining", fontsize=12)
    plt.title("Exponential Decay of Radioactive Elements", fontsize=14, pad=20)
    plt.xlim(0, 20000)
    plt.ylim(0, 1)

    plt.plot(x, y1, label="C-14", color="red", linestyle="--", linewidth=2)
    plt.plot(x, y2, label="Ra-226", color="green", linestyle="-", linewidth=2)

    plt.legend(loc="upper right", fontsize=12)
    plt.tight_layout()
    plt.show()
