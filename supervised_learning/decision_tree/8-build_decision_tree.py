...
class Decision_Tree():
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        """
        Computes the Gini impurity for a single feature and returns the best
        threshold and impurity.
        """
        # Get the feature values for the current node's sub_population
        feature_values = self.explanatory[node.sub_population, feature]
        # Get the target values for the current node's sub_population
        target_values = self.target[node.sub_population]
        # Get the possible thresholds for the current feature
        thresholds = self.possible_thresholds(node, feature)

        # Initialize arrays to store Gini impurities
        gini_impurities = np.zeros(len(thresholds))

        for i, threshold in enumerate(thresholds):
            # Split the population into left and right children
            left_mask = feature_values > threshold
            right_mask = ~left_mask

            # Get the target values for the left and right children
            left_counts = np.bincount(target_values[left_mask])
            right_counts = np.bincount(target_values[right_mask])

            # Calculate proportions for each class in left and right children
        ...
        # Find the best threshold (minimum Gini impurity)
        if len(thresholds) > 0:
