...
class Neuron():
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05,
        ...
        Args:
            ...
            alpha (float): learning rate
            verbose (bool): if True, prints the cost every 'step'
            iterations
            graph (bool): if True, plots the training
                          progress as a line graph
            step (int): step size for printing
        Returns:
            np.ndarray: the neuron’s prediction labels for
                        each example in X labelled 1 if the output
                        of the neuron is >= 0.5 and 0 otherwise
    ...
    @staticmethod
    def plot_training(cost, step):
        """
        Plots the training progress
        Args:
            cost (list): list of costs at each step
            step (int): step size for printing
        """
